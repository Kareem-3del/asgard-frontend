...
import {MdNavigateNext, MdNavigateBefore, AiFillDelete} from "react-icons/all";
import {FaCoins} from "react-icons/fa";
import useAuth from "../../hooks/auth/useAuth.hook";
import {toHtml} from "@fortawesome/fontawesome-svg-core";
import {getDeletedChapter} from "../../services/chapter.service";
            ...
            <div className="modal" dir="rtl">
                <div className="modal-box">
                    <h2 className="inline-flex mb-5">
                        شراء الفصل بـ <span className="mx-1 text-accent font-bold">10</span> نقاط
    ...
    return (
        <>
            ...
            <div
                ...
                {
                    chapter && (!chapterBought) && chapter.short_link  && <BuyChapterModal handelChapterBuy={handelChapterBuy} chapter={chapter}/>
                }
                <br/>
                <br/>
                {chapter && chapters && <ChapterNavigator chapter={chapter} chapters={chapters}/>}
...
const ChapterNavigator = ({chapter: chapter, chapters}: { chapter: IChapter, chapters: IChapter[] }) => {
    ...
    return (<div className="w-full mb-5 flex">
        <button className='btn w-12 h-12'><MdNavigateBefore/></button>
        <div className=" w-fit mx-auto flex items-center space-x-2">
            {chapter && <ChapterControl chapter={chapter}/>}
            <div className="dropdown">
                <label tabIndex={0} className="btn bg-base-300 m-1 w-52 h-12">الفصول</label>
                <ul tabIndex={0}
                    className="dropdown-content ml-1 menu p-2  shadow bg-base-300 rounded w-52">
                    {
                        chapters?.map((chapter_, index) => {
                            return (


                                <div key={`chapter_${chapter_.id}_${chapter_.number}`} className="max-h-[260px]  overflow-y-scroll flex flex-col overflow-hidden">
                                    <button onClick={() => {
                                        navigate(`/manga/${mangaSlug}/${chapter_.number}`)
                                    }}
                                            value={chapter_.number}
                                            className={`btn rounded btn-primary btn-xs m-1`}
                                            key={index}>
                                        {chapter_.number} - {"الفصل"}


                                        {chapter_.short_link ? "" : "LOOKED"}
                                    </button>
        ...
        <button className='btn w-12 h-12'><MdNavigateNext/></button>
    </div>)
}
